...
async def test_destructive_submit_locked_until_typed(
    ...
    try:
        ...
        # _seed_divergent now uses app.vault.create() so the local file is
        # guaranteed in app.vault.getFiles() before computeSyncPlan runs;
        # both destructive options must render with non-zero counts.
